...
如果您希望一次性安装所有常用的C/C++开发工具，可以使用`build-essential`软件包。它是一个元软件包，包含了GCC、G++、Make以及其他一些常用的开发库和工具。这是一种更便捷的安装方式，但不适合初学者，因为如果想要学习配置开发环境并且让自己有一个深刻的印象的话这个方式显然不行，他无法让你深刻的记住每个必备的组件都是什么。
...
首先，创建一个名为 `hello_world.cpp` 的文件，并写入以下代码：
